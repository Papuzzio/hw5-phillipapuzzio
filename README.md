...
On my first two push attempts of this repo, **GitHub Secret Scanning Push Protection rejected the push**, flagging strings produced by this skill as real Stripe and Slack credentials. This is the most direct possible validation that the skill works.

The first push attempt included the raw token files generated by the test runs. Push Protection blocked it with violations like:
...
remote:        locations:
remote:          - path: examples/decoys/stripe_sk_1.txt
remote:          - path: examples/decoys-edge/stripe_sk_1.txt
remote:          - path: examples/decoys-edge/stripe_sk_2.txt
remote:     
remote:      —— Slack Bot Token ——
remote:        locations:
remote:          - path: examples/decoys/slack_xoxb_1.txt
remote:          - path: examples/decoys-edge/slack_xoxb_1.txt
remote:          - path: examples/decoys-edge/slack_xoxb_2.txt
```

A total of 9 token-shaped strings were flagged. GitHub categorized them with explicit provider labels — "Stripe Live API Secret Key" and "Slack Bot Token" — meaning GitHub's scanner did not just see "looks like a secret" but specifically classified the strings by provider.

After gitignoring the raw token files, the second push attempt was *also* blocked. Push Protection flagged the structural example tokens I had written by hand into `references/token-formats.md` to illustrate each provider's format. GitHub's scanner could not tell those apart from real credentials either; they have since been sanitized with `EXAMPLE` substrings that visibly break the provider validator regex while preserving the structural diagrams.

The thesis of `honeytoken-forger` is that a script must compute provider-specific structural and checksum constraints correctly — otherwise the generated decoys are silently dropped by real scanners and never trigger any tripwire. GitHub's first-party scanner is the most-deployed secret scanner on Earth, and it could not distinguish these decoys from real credentials. The two rejection events above are independent third-party evidence that the skill's CRC32-Base62 math, charset constraints, and length specifications are correct.

Push Protection blocks are not preserved on GitHub's server-side Secret Scanning alerts page (the "no secrets found" status there is correct — Push Protection rejects uploads before they reach the scanner). The block events themselves were captured in the terminal output during the rejected push attempts; the sanitized files in this repo are the durable artifact.
